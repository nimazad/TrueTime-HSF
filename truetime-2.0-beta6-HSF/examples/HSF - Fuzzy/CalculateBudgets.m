function ServerBudgets = CalculateBudgets(TaskPeriods, TaskExecTimes, TaskNames, ServerPeriods, ServerBudgets, ServerNames)
numberOfServers = length(ServerPeriods);
for server=1:numberOfServers
    while  ~budgetIsEnough(server, TaskPeriods, length(TaskPeriods(1,:)), TaskExecTimes, ServerPeriods, ServerBudgets)
        ServerBudgets(server) = ServerBudgets(server) + ServerBudgets(server)*0.05;
    end
end
end


function result = budgetIsEnough(server, TaskPeriods, NumberOfTaskInServer, TaskExecTimes, ServerPeriods, ServerBudgets)
result = 1;
LCM=lcms(TaskPeriods(server,:));
for time=min(min(TaskPeriods)):LCM
    demand = 0;
    for task=1:NumberOfTaskInServer
        demand = demand + dbf(time, TaskPeriods(server, task), TaskExecTimes(server, task));
    end
    resource = sbf(time, ServerPeriods(server), ServerBudgets(server));
    if demand>resource
        result = 0;
        return;
    end
