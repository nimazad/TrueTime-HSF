...
for Generation=1:MaxGeneration
    ...
    for i=1:NumberOfIndividuals
        ...
        fitness(i) = sum(finish-(idle./MeanBudgets));
    end
    ...
    for i = 2:NumberOfIndividuals/2
        ...
        %CrossOver
        CrossOverPoint = randint(1,1,[1+gene_offset,NumberOfGenes+gene_offset]);
        Population(i*2-1,[1 CrossOverPoint]) = Pool(Parents(1), [1 CrossOverPoint]);
        Population(i*2-1,[CrossOverPoint NumberOfGenes+gene_offset]) = Pool(Parents(2), [CrossOverPoint NumberOfGenes+gene_offset]);
        %Mutation
        MutationPoint = randint(1,1,[1+gene_offset,NumberOfGenes+gene_offset]);
        ...
        Population(i*2,[CrossOverPoint NumberOfGenes+gene_offset]) = Pool(Parents(1), [CrossOverPoint NumberOfGenes+gene_offset]);
        %Mutation
        MutationPoint = randint(1,1,[1+gene_offset,NumberOfGenes+gene_offset]);
