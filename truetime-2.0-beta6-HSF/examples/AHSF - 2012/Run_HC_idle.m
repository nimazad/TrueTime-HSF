...
for iterations=1:300
    h_history(iterations) = idle_h;
    %fprintf('    -----------------------------------------Iteration -------------------------------------------\n')
    %disp(h_history);
    Parameters = [dl_K_s dl_d1 dl_d2 idle_h idle_K_s idle_d1 idle_d2 idle_h];
    ...
    if sum(dlMiss)>125
        fitness(iterations) = 0;
    else
        fitness(iterations) = sum(finish-((idle./MeanBudgets)));
    end
    hold on
    plot(fitness,'k*')
    ...
    if fitness(iterations) >= PrevFitness
        Prev_h = idle_h;
        idle_h = idle_h + direction*0.01*idle_h;
    else
        direction = -direction;
        idle_h = Prev_h + direction*0.01*Prev_h;
    end
