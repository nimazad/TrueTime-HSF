...
#include "getremainedcputime.cpp"
...
#include "getinvoker.cpp" //Added by Nima
#endif
...
double runKernel(double externalTime) {
  ...
  Task *task, *temp, *newrunning, *oldrunning;
  UserTask *usertask;
  ...
  *((long *)mxGetPr(rtsys->rtsysptr)) = (long)rtsys;
  ...
  while (timestep < TT_TIME_RESOLUTION) {
	...
	if (task->execTime / rtsys->cpuScaling < TT_TIME_RESOLUTION) {
	  ...
	  if (task->execTime < 0.0) { 
	    ...
	    //debugPrintf("'%s': task '%s' finished\n", rtsys->blockName, task->name);
		  if (task->isUserTask())
		  //debugPrintf("ttkernel:exectime < 0,task name = %s at %5.8f exectime = %5.8f\n", task->name, rtsys->time, task->execTime);
	    task->execTime = 0.0;
	    task->segment = 0;
	    task->nextSegment = 1;
    ...
    for (int i=0; i<rtsys->nbrOfCPUs; i++) 
	{
		...
		/*//Nima Major change old code end
		 ...
		 *///Nima Major change old code end
		  //Nima Major change new code start
		int firstTimeLoop = 1;
		while(firstTimeLoop || newrunning != oldrunning)
			{
			firstTimeLoop = 0;
		  ...
		  newrunning = (Task*) rtsys->readyQs[i]->getFirst(); // hooks may have released handlers
		}//end while newrunning != oldrunning
		  //Nima Major change new code end
		  //Nima
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
 ...
 //Added by Nima
  ...
  /* Server schedule outputs */
  dn = (DataNode*) rtsys->cbsList->getFirst();
  while (dn != NULL) 
  {	
    cbs = (CBS*) dn->data; 
    double val = (double) (i+5);
	int j;
    for (j = 0; j < rtsys->nbrOfCPUs; j++) 
	{
		s[i + j * rtsys->nbrOfSchedTasks] = val;
    }
    s[i + cbs->affinity * rtsys->nbrOfSchedTasks] = val + cbs->cs;
    i++;
    dn = (DataNode*) dn->getNext();
  }
} 
