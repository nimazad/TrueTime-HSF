...
int prioCmp(Node* n1, Node* n2) {
  ...
  if (t1->isUserTask()) {
	...
	//Added by Nima
	//When tasks belong to the same server --> compare them based on their deadline
    if(rtsys->isHSF && u1->cbs == u2->cbs)
	{
	      //debugPrintf("**************************-------both tasks are in '%s'\n", u1->cbs->name);
		  return rtsys->prioHEDF(u1) < rtsys->prioHEDF(u2);
	}
